...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-i", "--initial", action="store_true", help="initialize your information"
    )
    parser.add_argument(
        "-u", "--update", action="store_true", help="update the course score"
    )
    parser.add_argument(
        "-ls",
        "--list",
        nargs="*",
        metavar=("YEAR", "SEMESTER"),
        help="list the course and score in a certain year/semester",
    )
    parser.add_argument(
        "-n", "--name", nargs="+", help="search score by the name of the course"
    )
    parser.add_argument(
        "-g",
        "--gpa",
    ...
    parser.add_argument(
        ...
        help="set your DingTalk Robot Webhook. Empty means disabled",
    )
    parser.add_argument(
        "-dn",
        "--dnotification",
        action="store_true",
        help="enable dingtalk score notification",
    )
    ...
    if args.initial:
        ...
        try:
            ...
            print_log(
                LOG.error,
                "Cannot connect to the Internet. Please check your Internet connection.",
            )
        else:
    ...
    if args.update:
        ...
        try:
            ...
            print_log(
                LOG.error, "Cannot find your user data. Please use -i to initialize."
            )
            sys.exit()
        ...
        try:
            ...
            if not res:
                print_log(
                    LOG.error,
                    "Login failed. Please check your username and password. Remember to use -i to reset them.",
                )
            else:
                # 打开成绩查询网站
        ...
        try:
            ...
            print_log(
                LOG.error, "Cannot find your score data, please use -u to update first."
            )
    ...
    if args.list != None:
        if len(args.list) == 0:
            ...
            if len(courses) == 0:
                print_log(
                    LOG.info, f"Cannot find any courses during the whole college."
                )
            ...
            if len(courses) == 0:
                print_log(
                    LOG.info,
                    f"Cannot find any courses about the academic year of {args.list[0]}.",
                )
                print_log(LOG.tips, "Maybe you need to use -u to update first :)")
            else:
                print(f'{"Semeter":16s}{"Name":20s}\tMark\tGP\tCredit')
            ...
            if len(args.list) > 2:
                print_log(
                    LOG.warning,
                    f'The following argument(s) will be ignored:\n\t{" ".join(args.list[2:])}',
                )

            courses = [
                i
                for i in data.values()
                if i.get("xn").find(args.list[0]) == 0
                and args.list[1].find(i.get("xq", "-1")) != -1
            ]

            if len(courses) == 0:
                print_log(
                    LOG.info,
                    f'Cannot find any courses about the semester of {" ".join(args.list[:2])}',
                )
                print_log(LOG.tips, "Maybe you need to use -u to update first :)")
            else:
                print(f'{"Semeter":16s}{"Name":20s}\tMark\tGP\tCredit')
    ...
    if args.name:
        ...
        for searchcourse in args.name:
            res += difflib.get_close_matches(
                searchcourse, coursename, n=len(coursename), cutoff=0.3
            )
        ...
        if len(res) == 0:
            print_log(
                LOG.info,
                f'Cannot find any course matching keyword(s) {" ".join(args.name)}',
            )
        else:
            print(f'{"Semeter":16s}{"Name":20s}\tMark\tGP\tCredit')
            for name in res:
                    ...
                    if course.get("kcmc") == name:
                        print(
                            "{0:<{len0}}{1:<{len1}}\t{2}\t{3}\t{4}".format(
                                ...
                                course.get("xf"),
                                len0=pad_len(
                                    f"{course.get('xn')} {course.get('xq')}", 16
                                ),
                                len1=pad_len(course.get("kcmc"), 20),
                            )
                        )
    ...
    if args.gpa != None:
        if len(args.gpa) == 0:
            ...
            if len(grade) == 0:
                ...
                print_log(
                    LOG.done,
                    f"Your GPA during the whole college is {gpa:.3f} and GP is {gp:.2f}",
                )
            ...
            if len(grade) == 0:
                print_log(
                    LOG.info,
                    f"Cannot find any courses about the academic year of {args.gpa[0]}",
                )
                print_log(LOG.tips, "Maybe you need to use -u to update first :)")
            else:
                gp = 0.0
                ...
                print_log(
                    LOG.done,
                    f"Your GPA during the academic year of {year} is {gpa:.3f} and GP is {gp:.2f}",
                )
            ...
            grade = [
                ...
                for i in data.values()
                if i.get("xn").find(args.gpa[0]) == 0
                and args.gpa[1].find(i.get("xq", "-1")) != -1
                and i.get("cj") not in ["合格", "不合格", "弃修"]
            ]
            credit = [
                float(i.get("xf"))
            ...
            if len(grade) == 0:
                print_log(
                    LOG.info,
                    f'Cannot find any courses about the semester of {" ".join(args.gpa[:2])}',
                )
                print_log(LOG.tips, "Maybe you need to use -u to update first :)")
            else:
                gp = 0.0
                ...
                print_log(
                    LOG.done,
                    f"Your GPA during the semester of {year} {semster} is {gpa:.3f} and GP is {gp:.2f}",
                )
